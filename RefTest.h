...
A reference is an alias to an object resulting from the evaluation
...
bound. Also, if x is an lvalue, then it is *copied* to r; otherwise,
if it is an rvalue, then it is *moved* to r. Thus, in expressions
(14)-(17) and (19), the assignment invokes the copy operator (which
takes as parameter a const lvalue reference), and in expressions (18)
and (20)-(21), the move operator (which takes as parameter a rvalue
reference, ie, a reference for a temporary). (Attempting to use r8
after expression (20) should cause a compiler warning (at least)
since x1 has been moved, ie, it is supposed that its properties have
been "stolen", although the state of the object is still valid.)
*/
